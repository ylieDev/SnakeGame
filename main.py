...
class Apple:
    ...
    def move(self, snake_x, snake_y):
        while (self.x, self.y) in zip(snake_x, snake_y):
            self.x = random.randint(0, 24)*SIZE
            self.y = random.randint(0, 19)*SIZE
...
class Game:
    def __init__(self):
        ...
        self.surface = pygame.display.set_mode((SCREEN_WIDTH,SCREEN_HIGHT))
    ...
    def play(self):
        ...
        # snake coliding with edge
        if self.snake.x[0] < 0 or self.snake.x[0] > SCREEN_WIDTH-SIZE or self.snake.y[0] < 0 or self.snake.y[0] > SCREEN_HIGHT-SIZE:
            self.play_sound("crash")
            raise "Game Over"
    ...
    def run(self):
        ...
        while running:
            for event in pygame.event.get():
                if event.type == KEYDOWN and pygame.time.get_ticks()-self.keytime >=100:
                    self.keytime = pygame.time.get_ticks()
